...
class ResearchConductor:
    ...
    def evidence_metrics(self) -> dict[str, Any]:
        ...
        for work_item in self.research_work_items:
            ...
            for evidence_query, evidence in self._evidence_by_query.items():
                if evidence_query == work_item.query or evidence_query.startswith(
                    prefix
                ):
    ...
    def _codex_execution_peak(self) -> int:
        ...
        for run in self._codex_run_metadata:
            ...
            try:
                events.append(
                    (datetime.fromisoformat(started.replace("Z", "+00:00")), 1)
                )
                events.append(
                    (datetime.fromisoformat(finished.replace("Z", "+00:00")), -1)
                )
            except ValueError:
                continue
        active = 0
    ...
    def _normalize_work_items(
        ...
        for candidate in candidates:
            if isinstance(candidate, dict):
                query = str(
                    candidate.get("query") or candidate.get("task") or ""
                ).strip()
                tags = self._string_tuple(
                    candidate.get("coverage_tags") or candidate.get("tags")
                )
    ...
    def _compact_web_retriever_query(query: str) -> str:
        ...
        return re.sub(r"\s+", " ", str(query)).strip()[:380]

    def _lightweight_web_retriever_queries(self, query: str) -> list[str]:
        """Return one domain-neutral query for ordinary web retrievers."""

        return [self._compact_web_retriever_query(query)]
    ...
    async def conduct_research(self):
        ...
        has_mcp_retriever = any(
            "mcpretriever" in r.__name__.lower() for r in self.researcher.retrievers
        )
        if has_mcp_retriever:
            self.logger.info(
                "MCP retrievers configured and will be used with standard research flow"
            )
        ...
        # Hybrid search including both local documents and web sources
            ...
            docs_context = " ".join(str(part) for part in docs_context_parts if part)
        ...
        self.researcher.codex_run_metadata = [
            run.copy() for run in self._codex_run_metadata
        ]
    ...
    async def _get_context_by_web_search(
        self, query, scraped_data: list | None = None, query_domains: list | None = None
    ):
        """
        ...
        if mcp_retrievers and self._mcp_results_cache is None:
            if mcp_strategy == "disabled":
                ...
                self.logger.warning(
                    f"Unknown MCP strategy '{mcp_strategy}', defaulting to fast"
                )
                mcp_context = await self._execute_mcp_research_for_queries(
                    [query], mcp_retrievers
                )
        ...
        async def run_work_item(item: ResearchWorkItem):
            try:
                return await self._process_sub_query(
                    item.query, scraped_data, query_domains
                )
            finally:
    ...
    def _build_gap_followups(
        ...
        *,
        ...
        if total_sources < minimum_total and len(followups) < 3:
            ...
            for item in sorted(
                work_items,
                key=lambda work_item: self._query_http_source_counts.get(
                    work_item.query, 0
                ),
            ):
                followup = (
        ...
        return max(
            ...
            int(
                os.getenv(
                    ...
                    os.getenv("RESEARCH_MIN_TOTAL_HTTP_SOURCES", "2"),
                )
            ),
        )
    ...
    def _find_evidence_conflicts(self) -> list[dict[str, Any]]:
        ...
        for item in self.evidence_items:
            ...
            if not decimal_value.is_finite():
                continue
            normalized_value = (
                "0" if decimal_value == 0 else format(decimal_value.normalize(), "f")
            )
    ...
    def _get_mcp_strategy(self) -> str:
        ...
        if (
            hasattr(self.researcher, "mcp_strategy")
            and self.researcher.mcp_strategy is not None
        ):
    ...
    async def _execute_mcp_research_for_queries(
        self, queries: list, mcp_retrievers: list
    ) -> list:
        """
        ...
        try:
            ...
            if mcp_retrievers:
                if mcp_strategy == "disabled":
                    ...
                    self.logger.warning(
                        "MCP cache not available, falling back to per-sub-query execution"
                    )
                    if self.researcher.verbose:
                        await stream_output(
                            "logs",
            ...
            if scraped_data:
                ...
                self.logger.info(
                    f"Web content found for sub-query: {len(str(web_context)) if web_context else 0} chars"
                )
            ...
            if combined_context:
                ...
                if self.researcher.verbose:
                    ...
                    cache_used = (
                        self._mcp_results_cache is not None
                        and mcp_retrievers
                        and mcp_strategy != "deep"
                    )
                ...
                self.logger.warning(
                    f"No combined context found for sub-query: {sub_query}"
                )
                if self.researcher.verbose:
                    await stream_output(
                        "logs",
    ...
    async def _execute_mcp_research(self, retriever, query):
        ...
        try:
            ...
            if results:
                ...
                self.logger.info(
                    f"No results returned from MCP research with {retriever_name}"
                )
                if self.researcher.verbose:
                    await stream_output(
                        "logs",
    ...
    def _combine_mcp_and_web_context(
        self, mcp_context: list, web_context: str, sub_query: str
    ) -> str:
        """
        ...
        if combined_parts:
            final_context = "\n\n".join(combined_parts)
            self.logger.info(
                f"Combined context for '{sub_query}': {len(final_context)} total chars"
            )
    ...
    async def _process_sub_query_with_vectorstore(
        self, sub_query: str, filter: dict | None = None
    ):
        ...
        context = await self.researcher.context_manager.get_similar_content_by_query_with_vectorstore(
            sub_query, filter
        )
            ...
            try:
                async with semaphore:
                    ...
                    if is_codex:
                        ...
                        self._codex_calls += 1
                        initial_query_keys = {
                            self._query_key(item.query)
                            for item in self.research_work_items
                        }
                        ...
                        results = []
                        retriever_queries = self._lightweight_web_retriever_queries(
                            query
                        )
        ...
        for retriever_name, results in gathered:
            for result in results:
                ...
                dedupe_key = (
                    canonical_url
                    or url
                    or f"{retriever_name}:{result.get('title', '')}:{result.get('body', '')[:120]}"
                )
    ...
    def _merge_duplicate_search_result(
        ...
        if incoming_body and incoming_body not in existing_body:
            existing["body"] = "\n\n".join(
                part for part in (existing_body, incoming_body) if part
            )
                ...
                for item in structured:
                    ...
                    try:
                        evidence_kwargs: dict[str, Any] = dict(
                            ...
                            summary=str(
                                item.get("summary") or result.get("body") or ""
                            ),
                        )
            ...
            if hasattr(retriever_instance, "search"):
                ...
                if results:
                    ...
                    if is_mcp_retriever:
                        ...
                        if result_count > 3:
                            self.logger.info(
                                f"... and {result_count - 3} more MCP results"
                            )
                else:
                ...
                self.logger.error(
                    f"Retriever {retriever_name} does not have a search method"
                )
                return []
        except Exception as e:
